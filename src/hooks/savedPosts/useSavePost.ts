      ...
      if (data.savedPosts) {
        return {
          ...
          savedPosts: data.savedPosts.map((item: any) => {
            ...
            const hasExperience =
              item.experience && item.experience.id === postID;
            ...
            if (hasExperience) {
              return {
                ...item,
                experience: { ...item.experience, isSaved: true },
              };
            }
            if (hasBlog) {
              return {
...
const updateQueryData = (
  ...
  queryClient.setQueriesData({ queryKey: [queryKey] }, (old: any) => {
    ...
    const handler = isSavedPostsStructure
      ? dataStructureHandlers.savedPosts
      : dataStructureHandlers.regular;
  ...
  const savePost = useMutation({
    mutationFn: async ({ postID, userID, type, queryKey }: SavePostParams) => {
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/save-post`,
        {
          postID,
          userID,
          type,
      ...
      return {
        ...response.data,
        type: response.data.postType,
        queryKey,
        postID,
      }; // Return postType and queryKey for onSuccess
    },
    onSuccess: async (variables) => {
      ...
      if (queryKey) {
        ...
        const updatedCount = updateQueryData(
          queryKey,
          postID,
          "save",
          queryClient
  ...
  const unsavePost = useMutation({
    mutationFn: async ({ savedPostID, queryKey }: UnsavePostParams) => {
      const response = await axios.delete(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/unsave-post/${savedPostID}`
      );
