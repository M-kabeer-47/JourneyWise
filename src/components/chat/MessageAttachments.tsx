...
const downloadFile = async (url: string, filename: string) => {
  try {
    // Fetch the file as a blob to avoid navigation
          ...
          >
            ...
            {attachment.isUploading && count === 1 && (
              <div className="absolute inset-0 bg-black/40 flex items-center justify-center">
                <div className="text-center">
                  ...
                  <div className="text-white text-sm font-medium font-geist">
            ...
            {!attachment.isUploading && (
              <button
                ...
                className="absolute top-2 right-2 p-1.5 bg-white shadow-lg rounded-full transition-all opacity-0 group-hover:opacity-100"
                title="Download"
              >
                <Download className="h-3.5 w-3.5 text-ocean-blue" />
              </button>
            )}
          </div>
        ))}
        ...
        {isUploading && count > 1 && (
          <div className="absolute inset-0 bg-black/40 rounded-lg flex items-center justify-center">
            <div className="text-center">
              ...
              <div className="text-white text-base font-medium font-geist">
...
const FileAttachmentList: React.FC<{
  ...
  const getFileIcon = (fileName: string) => {
    ...
    return <File className="h-5 w-5 text-charcoal/60" />;
  };
    ...
    <div className="space-y-2">
      {attachments.map((attachment) => {
        ...
        return (
          <div
            key={attachment.id}
            className={`flex items-center gap-3 p-3 rounded-lg w-80 font-geist ${
              isOwnMessage ? "bg-white/10" : "bg-gray-50 border border-gray-200"
            }`}
          >
            {/* File Icon or Loader */}
              ...
              {isNameTooLong ? (
                <Popover open={isHovering} onOpenChange={setIsHovering}>
                  ...
                  <PopoverContent className="w-auto max-w-md p-2">
                    <p className="text-sm break-all font-geist">
                      {attachment.name}
                    </p>
                  </PopoverContent>
                </Popover>
            ...
            {!attachment.isUploading && (
              <button
                ...
                className={`p-1 rounded-lg transition-colors ${
                  isOwnMessage ? "hover:bg-white/10" : "hover:bg-gray-100"
                }`}
                title="Download"
              >
                <Download
