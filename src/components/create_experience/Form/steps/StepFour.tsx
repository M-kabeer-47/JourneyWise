...
export default function FormStep4({
  ...
  const addTier = () => {
    if (formData.tiers.length < 3) {
      const newTiers = [
        ...formData.tiers,
        { name: "", members: 0, price: 0, description: "" },
      ];
  ...
  const removeTier = (index: number) => {
    if (formData.tiers.length > 1) {
      const newTiers = formData.tiers.filter(
        (_, i) => i !== formData.tiers.length - index - 1
      );
      handleInputChange("tiers", newTiers);
      setActiveTierIndex(formData.tiers.length - index - 1);
    }
              ...
              >
                <div className="flex items-center justify-between bg-gray-50 -mx-6 -mt-6 px-6 py-3 border-b border-gray-200 mb-4">
                  <h4 className="text-lg font-medium text-midnight-blue">
                    Tier {formData.tiers.length - index}
                  </h4>
                ...
                <div className="space-y-4">
                  <input
                    ...
                    onFocus={() => {
                      ...
                      handleFocus(
                        `tiers.${formData.tiers.length - index - 1}.name`
                      );
                    }}
                    onBlur={handleBlur}
                    className={`w-full px-4 h-11 rounded-lg border text-charcoal text-sm
                  ...
                  />
                  {formData.tiers?.[formData.tiers.length - index - 1]?.name ===
                    "" &&
                    submit && (
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      onFocus={() => {
                        handleTierFocus(index);
                        handleFocus(
                          `tiers.${formData.tiers.length - index - 1}.price`
                        );
                      }}
                      min={1}
                      onBlur={handleBlur}
                  ...
                  {formData.tiers?.[formData.tiers.length - index - 1]
                    ?.price === 0 &&
                    submit && (
                      <p className="text-red-500 text-sm mt-1">
                        Price is required
                  ...
                  {formData.tiers?.[formData.tiers.length - index - 1]?.price <
                    1 &&
                    formData.tiers[formData.tiers.length - index - 1].price !==
                      0 &&
                    submit && (
                  ...
                  <input
                    ...
                    onFocus={() => {
                      handleTierFocus(index);
                      handleFocus(
                        `tiers.${formData.tiers.length - index - 1}.members`
                      );
                    }}
                    onBlur={handleBlur}
                    className={`w-full px-4 h-11 rounded-lg border text-charcoal text-sm
                  ...
                  <textarea
                    ...
                    onFocus={() => {
                      handleTierFocus(index);
                      handleFocus(
                        `tiers.${formData.tiers.length - index - 1}.description`
                      );
                    }}
                    onBlur={handleBlur}
                    maxLength={100}
            ...
            <div className="flex items-center gap-4">
              <TooltipProvider>
                <Tooltip>
                  ...
                  <TooltipContent className="bg-white text-midnight-blue border border-gray-200 p-1 rounded-lg text-[13px]">
                    <p>
                      List any requirements or prerequisites for participants
                    </p>
                  </TooltipContent>
                </Tooltip>
              </TooltipProvider>
