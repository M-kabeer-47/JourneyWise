...
export default function FormStep2({
  ...
  const getFieldError = (day: number, activityIndex: number, field: string) => {
    return errors.destinations?.[day - 1]?.activities?.[activityIndex]?.[
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      updateDestinationName(destination.id, e.target.value)
                    }
                    onFocus={() =>
                      handleFocus(`destinations.${destination.day - 1}.name`)
                    }
                    onBlur={handleBlur}
                    className={`w-full px-4 h-11 rounded-lg border text-charcoal text-sm
                             ...
                             ${
                               focusedField ===
                               `destinations.${destination.day - 1}.name`
                      ...
                      (activity: Activity, activityIndex: number) => (
                        ...
                        >
                                ...
                                const newDestinations =
                                  formData.destinations.map((dest) =>
                                    dest.id === destination.id
                                      ? {
                                          ...dest,
                                          activities: dest.activities.map(
                                            (act) =>
                                              act.id === activity.id
                                                ? {
                                                    ...act,
                          ...
                          <button
                            type="button"
                            onClick={() =>
                              removeActivity(destination.id, activity.id)
                            }
