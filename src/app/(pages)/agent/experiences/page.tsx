...
export default function AgentExperiences() {
  ...
  const [currentPage, setCurrentPage] = useState(parseInt(current.get("page") || "1"));
  const [sortBy, setSortBy] = useState(current.get("sortBy") || "createdAt");
  const [sortOrder, setSortOrder] = useState(current.get("sortOrder") || "desc");
          ...
          <div className="px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
            ...
            <div className="mt-8 grid grid-cols-2 sm:grid-cols-4 gap-4">
              <div className="bg-white/10 backdrop-blur-sm rounded-lg p-4">
                ...
                <p className="mt-1 text-2xl font-bold">{totalExperiences || 0}</p>
              </div>
              
              <div className="bg-white/10 backdrop-blur-sm rounded-lg p-4">
                <p className="text-sm font-medium text-blue-100">Available</p>
                <p className="mt-1 text-2xl font-bold">
                  {experiences && experiences.filter(exp => exp.isAvailable).length || 0}
                </p>
              </div>
            </div>
          </div>
            ...
            <p className="text-sm text-gray-500">
              {!isLoading && (
                <>
                  ...
                  }</span> of <span className="font-medium text-gray-900">{totalExperiences || 0}</span> experiences
                </>
              )}
            </p>
          </div>
